...
export class Cart {
  constructor(count) {
    this.countElement = document.querySelector('.counter');
    this.btnEl = document.querySelectorAll('.product__btn');
    this.count = count;
    this.items = [];
  }
    ...
    this.btnEl.forEach((btn) =>
      btn.addEventListener('click', (e) => {
        ...
        if (localStorage.getItem('cart')) {
          ...
          // this.items = JSON.parse(ls).items;
        ...
        const id = +item.querySelector('.product__info div:first-child span').textContent.trim();
        this.count += 1;
        this.getCount(this.count);
        let ls;

        this.items.push(...catalogData.filter((el) => el.id === id));
